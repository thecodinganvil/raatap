...
export default function SetPasswordContent() {
  ...
  // Check if user has a valid session (they should, since they clicked the email link)
  useEffect(() => {
    const checkSession = async () => {
      ...
      // If user already set their password, redirect to dashboard
      const passwordAlreadySet = session.user?.user_metadata?.password_set === true;
    ...
    // Also listen for auth state changes (in case session comes in via URL)
    const {
      data: { subscription },
  ...
  const handleSetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!session) {
        setError(
          "Your session has expired. Please sign up again or log in."
        );
        setNoSession(true);
        return;
      }

      // Set the password AND mark password_set in user metadata
      // This flag is checked by dashboard/signup to prevent access without a password
      ...
      <main className="min-h-screen bg-gradient-to-br from-[#f0f2ff] via-[#f8f9fc] to-[#e8ebff] flex items-center justify-center px-4 py-8">
        ...
        <div className="relative w-full max-w-lg text-center">
          <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl shadow-[#6675FF]/10 p-8 md:p-10 border border-white/50">
            <div className="w-20 h-20 mx-auto mb-6 rounded-full bg-amber-100 flex items-center justify-center text-amber-500">
              <svg
                className="w-10 h-10"
                fill="none"
                stroke="currentColor"
            ...
            <h1 className="text-2xl font-medium text-[#171717] mb-3">
              Session Expired
            </h1>
            <p className="text-gray-500 mb-8">
              Your session has expired or is invalid. Please sign up again or
        ...
        <div className="relative w-full max-w-lg">
          <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl shadow-[#6675FF]/10 p-8 md:p-10 border border-white/50 text-center">
            ...
            <p className="text-gray-500 mb-4">
              Your password has been created successfully. Redirecting you to
              your dashboard...
            </p>
            <div className="flex justify-center">
              <div className="animate-spin w-6 h-6 border-3 border-[#6675FF] border-t-transparent rounded-full"></div>
            </div>
          </div>
        </div>
      </main>
      ...
      <div className="relative w-full max-w-lg">
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl shadow-[#6675FF]/10 p-8 md:p-10 border border-white/50">
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            {/* Verified Badge */}
            <div className="flex items-center gap-2 px-4 py-2 bg-green-50 border border-green-200 rounded-full mb-4">
              <svg
            ...
            <div>
              ...
              {password && (
                <div className="mt-2 space-y-1">
                  ...
                  <p
                    className={`text-xs ${passwordStrength < 3 ? "text-amber-600" : "text-green-600"}`}
                  >
                    Password strength:{" "}
